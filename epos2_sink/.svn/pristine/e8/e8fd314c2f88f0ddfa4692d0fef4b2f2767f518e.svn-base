    ...
    void init(){
        ...
        for(int i = 0; i < _scanner->_sector_list.size(); i++){
            ...
            if(available_space > 0) { //sector is empty
                ...
                db<Persistent_Storage>(TRC) << "  --> This sector is not empty" << endl;
            }
...
public:
    ...
    void read(char *buffer, uint32_t size){
        ...
        for(int i = 0; i < _data_list->size(); i++){
            if(buffer_position >= total_size) break;

            d = _data_list->get(i)->object();

            _device->read(d->_data_offset, &temp[buffer_position], d->_size);
            buffer_position += d->_size;
        }

        for(int i = 0; i < size; i++){
            *buffer = temp[_pointer + i];
            buffer++;
        }
    }
